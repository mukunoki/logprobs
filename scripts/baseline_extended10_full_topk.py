#!/usr/bin/env python3
"""
Baseline Extended 10 Full Top-k Evaluation

Extended 10問題でベースライン手法（k=3全候補テスト）を評価する。
...
OUTPUT_FILE = Path(__file__).resolve().parents[1] / "results" / "baseline_extended10_full_topk.json"
...
def run_baseline_extended10_full_topk(num_trials: int = 1) -> Dict:
    """Extended 10問題でのFull Top-kベースライン評価を実行"""
    print(f"\n{'='*80}")
    print(f"Baseline Extended 10 Full Top-k Evaluation ({num_trials} trial(s))")
    ...
    # 結果を保存
    output = {
        'method': 'baseline_extended10_full_topk',
...
if __name__ == '__main__':
    ...
    run_baseline_extended10_full_topk(num_trials)
