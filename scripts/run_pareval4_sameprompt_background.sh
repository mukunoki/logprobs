...
RUN_SCRIPT="${REPO_ROOT}/scripts/run_pareval4_sameprompt_all.sh"
cd "$REPO_ROOT"
...
LOG_PATH="${LOG_PATH:-${PAPER_DIR}/logs/pareval4_sameprompt_${TS}_${NUM_TRIALS}trials_w${WORKERS}_mt${MAX_TOKENS}.log}"
PID_PATH="${PAPER_DIR}/logs/pareval4_sameprompt_latest.pid"
META_PATH="${PAPER_DIR}/logs/pareval4_sameprompt_latest.meta"

if [[ -f "$PID_PATH" ]]; then
  old_pid="$(cat "$PID_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A pareval4 same-prompt experiment is already running with PID $old_pid" >&2
    exit 1
  fi
fi
...
echo "Started pareval4 same-prompt background run"
echo "pid: $pid"
echo "log: $LOG_PATH"
