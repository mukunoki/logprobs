...
LOG_PATH="${LOG_PATH:-${LOG_DIR}/sensitivity2_grid_${TS}_${NUM_TRIALS}trials_w${WORKERS}_mt${MAX_TOKENS}.log}"
PID_PATH="${PID_PATH:-${LOG_PATH%.log}.pid}"
META_PATH="${META_PATH:-${LOG_PATH%.log}.meta}"
RUN_SCRIPT="${SCRIPT_DIR}/run_sensitivity2_grid_all.sh"
...
export NUM_TRIALS TEMPERATURES BUDGETS MAX_TOKENS WORKERS CHECKPOINT_EVERY VLLM_USE_CHAT OUT_DIR
...
meta_path.write_text(
    "\\n".join([
        ...
        f"temperatures={env.get('TEMPERATURES')}",
        f"budgets={env.get('BUDGETS')}",
...
echo "[INFO] started sensitivity2 grid"
echo "[INFO] pid: $PID_PATH"
echo "[INFO] log: $LOG_PATH"
