...
RUN_SCRIPT="${REPO_ROOT}/scripts/run_hard4_prompt_check_all.sh"
cd "$REPO_ROOT"
...
LOG_PATH="${LOG_PATH:-${PAPER_DIR}/logs/hard4_prompt_minimal_${TS}.log}"
PID_PATH="${PAPER_DIR}/logs/hard4_prompt_minimal_latest.pid"
META_PATH="${PAPER_DIR}/logs/hard4_prompt_minimal_latest.meta"

if [[ -f "$PID_PATH" ]]; then
  old_pid="$(cat "$PID_PATH" 2>/dev/null || true)"
  if [[ -n "$old_pid" ]] && kill -0 "$old_pid" 2>/dev/null; then
    echo "A hard4 prompt check is already running with PID $old_pid" >&2
    exit 1
  fi
fi
...
echo "Started hard4 prompt check background run"
echo "pid: $pid"
echo "log: $LOG_PATH"
