...
OUT_DIR="${OUT_DIR:-${RESULTS_DIR}/paper12_parallel_${TS}}"
...
run_parallel_eval() {
  local benchmark="$1"
  local output="$2"
  echo "[INFO] $(date '+%F %T') start parallel threshold eval benchmark=${benchmark}"
  python3 "${SCRIPTS_DIR}/threshold_refinement_eval_parallel.py" \
    ...
    --resume \
    --output "$output"
  echo "[INFO] $(date '+%F %T') done parallel threshold eval benchmark=${benchmark}"
}

run_parallel_eval paper_easy4 "${OUT_DIR}/threshold_refinement_eval_paper_easy4_b${BUDGET}.json"
run_parallel_eval paper_medium4 "${OUT_DIR}/threshold_refinement_eval_paper_medium4_b${BUDGET}.json"
run_parallel_eval paper_hard4 "${OUT_DIR}/threshold_refinement_eval_paper_hard4_b${BUDGET}.json"
...
echo "[INFO] completed paper12 parallel experiments"
