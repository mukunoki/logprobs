...
\begin{abstract}
LLMによるVerilogコード生成では，候補を複数生成しても最終的にはコンパイル，elaboration，simulationを通したvalidationが必要であり，validation costが課題となる．本稿は，token log確率を候補全体の正誤スコアではなく，どこを重点的にvalidationすべきかを示す局所的不確実性信号として用いる手法Hybrid Uncertainty-Guided Testing（Hybrid-UGT）を提案する．低log確率token周辺のspanから入力信号名・bit範囲・演算子・case/state・reset/clockを抽出し，参照実装をoracleとするtestbench向けdirected validation inputを生成する．VerilogEval code-complete-iccad2023の compile-pass official-fail候補31,842件全件に対する評価では，Hybrid-UGTがRandom uniformをdet@1で$+0.028$上回り（95\% CI $[+0.022, +0.034]$，counter系n=2,375で$+0.146$），det@16では同等に収束する（$\Delta=-0.005$）．Hybrid-UGTは少数vectorでのfail-fast screening用途で有効である．
\end{abstract}
...
\input{verilog_eval_feature_table}
...
\input{verilog_eval_correlation_table}
...
\input{verilog_ugt_overall_full_table}
...
\input{verilog_ugt_struct_full_table}
